    ...
    useEffect(() => {
        if (connection) {
            ...
            connection.off('ClearMessages').on('ClearMessages', data => {
                ...
                setMessages([{
                    "system": true,
                    "message": "một người lạ đã rời phòng..."
                }]);
            });
            ...
            <div className="col-span-9 2xl:w-full">
                <div className="border-b border-zinc-500/5 flex items-center justify-between px-6 py-5 text-white">
                    <div className="flex items-center">
                        ...
                        <div className="ml-3">
                            <p className="text-lg font-medium flex items-center">{members.filter(a=>a).length == 1 ? <p>Chờ người lạ...</p> : <p>Anonymous Chat</p>}</p>
                            <p className="text-xs font-medium italic text-gray-500">{members.filter(a=>a).length == 1 ? <p>Đang kết nối với người lạ...</p> : <p>This room is with a stranger. </p>}</p>
                        </div>
                    </div>
                                ...
                                if (message.self) {
                                    ...
                                    >
                                        <div className="flex flex-col items-end">
                                            ...
                                            {!message.user.verified ? <p className="text-xs text-gray-500  p-1 mr-1 rounded-lg">Me</p> : <p className="text-xs text-gray-500  p-1 mr-1 rounded-lg flex items-center">Tô<FontAwesomeIcon className=" h-3 mx-1" icon={faCheckCircle} /></p>}
                                    ...
                                    >
                                        ...
                                        {message.user.verified ? <p className="text-xs text-gray-500  p-1 mr-1 rounded-lg flex flex-row items-center">{message.user.username != "Bot" ? "Stranger" : "Bot"}<FontAwesomeIcon className=" h-3 mx-1" icon={faCheckCircle} /> </p> : <p className="text-xs text-gray-500  p-1 mr-1 rounded-lg flex items-center">Người lạ</p>}
                                            <div className="bg-zinc-500/10 rounded-xl p-3">
                                            {message.file && <img className="h-[300px] rounded-lg justify-end mb-2 object-cover" src={Image(message.file, message.type)}/>}
                                                <p className="text-sm text-white break-all">{message.message}</p>  
                                                <p className="text-xs text-gray-500">{dateNow(message.date)}</p>
                     ...
                     {members?.map(member => (
                        <div> {member?.username != user?.username &&
                           ...
                           {istypping && typping?.user.username != user?.username && <img src={`${mainConfig.avatarAPI + member?.username.slice(0, 2) + id || "NoName"}.png`} alt="username" className="w-10 h-10 rounded-full" />}
                            {istypping && typping?.user.username != user?.username && <p className="text-xs text-gray-500 mt-2">Đang soạn...</p>}
                        </div>}
                               </div>
                ...
                <form onSubmit={e => {
                      ...
                      <motion.li className="flex flex-col space-y-4" layout initial='initial' animate='animate' exit='animate'> 
                        ...
                        <div className="flex items-center">
                            <input onChange={() => typpingu()} name="message" disabled={members.filter(a=>a).length == 1} type="text" className={members.filter(a=>a).length == 1 ? `bg-zinc-500/10 rounded-md w-full px-4 py-2 cursor-not-allowed text-white outline-none` : `bg-zinc-500/10 rounded-md w-full px-4 py-2 text-white outline-none`} autoComplete="off" placeholder={members.filter(a=>a).length == 1 ? "Chờ xíu nha..." : `Đang soạn...`} />
                    ...
                    {members?.map(member => (
                        <div> {member?.username != user?.username ?
                            ...
                            <div className="flex items-center w-full">
                                ...
                                <div className="ml-3 flex items-center justify-between gap-2 w-full"> {member?.verified ? <p className="text-sm font-medium flex flex-row items-center">Tôi <FontAwesomeIcon className=" h-3 mx-1" icon={faCheckCircle} /></p> : <p className="text-sm font-medium">Me</p>}
